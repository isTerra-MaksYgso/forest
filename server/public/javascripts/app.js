...
var autorizateForm = (function() {
	...
	var _ajaxForm = function (form, url) {
		if (!validation.validateForm(form, 5, 20)) {
...
var validation = (function() {
	...
	var _removeNeedClass = function () {
		...
		$(this).siblings('.login-error-box').css('display','none');

	};

	var clearForm = function (form) {
		var form = form;
		form.find('input, textarea').val("").css('background-color', '#fff');
...
var contactMeForm = (function() {
	...
	var _submitForm = function (e) {
		...
		sendAjaxJsonModule.prepareSendMail();
		validation.clearForm(form);
	};
...
var SliderModule = (function() {
	...
	$('.my-works-itself').eq(0).css('display','block');
...
var addblogModule = (function() {
	var init = function () {
		_setUpListeners();
	};

	var _setUpListeners = function () {
		$('#blogAddPost').children('.settings-save').on('click', prepareSendPost);
	}

	function prepareSendPost(){
		var data = {
			name: formBlog.name.value,
		...
		$.ajax({
			method: "POST",
			url: '/admin/blogAddArticle',
			data: data,
			success: function (data) {
				showAdminModal(data);
				validation.clearForm(formBlog);
			}
		});
	}
...
var SkillsModule = (function() {
	...
	var _start = function () {
		$('.my-skills__circle-svg-above').css('stroke-dasharray', '314');
		$('.my-skills__circle-svg-above').css('transform', 'rotate(-90deg)');
		$('.my-skills__circle-svg-above').css('transform-origin', '50% 50%');
		
		var i = 0;
		$('.admin-current-block__info')
			.find('.admin-settings__item-name')
			.each(function () {
				var _category = $(this).closest('.admin-current-block-settings').children('.admin-settings__name').text();
				var _name = $(this).text();
				var _val = $(this).children('.admin-settings__item-value').val();
				skillsArray.push({cat: _category, title: _name, val: _val});
				i++;
		});
	...
	// добавить прозрачность скрилам
	/*	var t = $('.skills-group').find('.my-skills__circle-span');
		console.log(t);*/
		// console.log(skillsArray);
	/*	for (var i = 0; i < t.length; i++) {
			for (var j = 0; j < skillsArray.length; j++) {
				if (t[i].text == skillsArray[j].title) {
					console.log(t[i].text);
					var valueSkills = skillsArray[j].val,
						needSkillValue = 314 * (valueSkills/100);
					t[i].css('stroke-dashoffset', needSkillValue);
					t[i].css('opacity', (1 * valueSkills / 100));
				}
			}
		};*/


	function _sendAjaxSkills(url, data, cb) {
		$.ajax({
			method: "POST",
			url: url,
			data: {skillsArray: data},
			success: function (data) {
				alert(data);
			}
		});
	...
	$('#aboutAdm').children('.settings-save').on('click', _prepareSendSkills);
	...
	function _prepareSendSkills(e) {
		...
		_sendAjaxSkills('/admin/skills', JSON.stringify(skillsArray), function (data) {
			alert(data);
		});
		return skillsArray;
	};
...
var sendAjaxJsonModule = (function() {
	var formMail = document.querySelector('#mailMeForm');
	...
	return {
		prepareSendMail: prepareSendMail,
		sendAjaxJson: sendAjaxJson
	};
})();

var showAdminModal = function (text) {
	$('.admin-modal-win__caption').text(text);
	$('.admin-modal-win').css('display','flex');
	$('.admin-modal-win__close').on('click', function () {
		$('.admin-modal-win').css('display','none');
	});
};
...
$(document).ready(function () {
	...
	$('.autorizate-button').on('click', function () {
		if ($(this).val() == 'auth') {
	...
	$('.header__down--about').on('click', function () {
		var topL = $('.about-me-left-section').offset().top;
		$('body, html').animate({scrollTop: topL}, 500);
	});

	$('.header__down--works').on('click', function () {
		var topL = $('.my-works-gossip').offset().top+50;
		$('body, html').animate({scrollTop: topL}, 500);
	});
	...
	$('.admin-tabs__item').on('click', function () {
		window.location.hash = $(this).children('a').attr('href').replace('#', '');
	...
	// вынести в отдельный модуль
		var formUpload = document.querySelector('#adminUploadFile');
		...
		function prepareSendFile(e) {
			e.preventDefault();
			var resultContainer = document.querySelector('.upload-file-admin-label');
			var formData = new FormData();
			var file = document.querySelector('#addImg-admin').files[0];
			...
			fileUpload('/admin/fileUp', formData, function (data) {
				resultContainer.innerHTML = data;
				validation.clearForm(formUpload);
			});
		}
});
